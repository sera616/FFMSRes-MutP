...
#     df = DataFrame({'Name': seq_name, 'Sequence': seq, 'Pos': pos,'wtAA': wtAA,'mutAA': mutAA,'Score': predict.flatten()},
#                index=range(len(seq_name))) #len(seq_name)：len(P26439),也就是index=6.这违背原意
    df = DataFrame({'Name': seq_name, 'Sequence': seq, 'Pos': pos,'WtAA': wtAA,'MutAA': mutAA,'Score': predict2.flatten()},index=range(len(SEQ_name))) ## SEQ_name是一个list，里面只有一个值，如"P02516"
...
#     jobid = '20210810232231'
#     # model_choose = 'AL6mA_th'
#     # # 'AL6mA_th''AL6mA_me''LA6mA_th''LA6mA_me'
# #     email = '1185540181@qq.com'
...
#         batch_size = 30
        ...
        # 邮件模块
        sendemail2.sendmail(jobid=jobid, email=email.strip(), job_path=data_path) ##没问题
